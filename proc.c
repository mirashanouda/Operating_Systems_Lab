...
found:
  p->state = EMBRYO;
  p->pid = nextpid++;
  p->priority = 100;   //set defalt priority here
...
scheduler(void)
{
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      c->proc = maxProc; //switch the current process running on the CPU to this runnable process
      ...
      swtch(&(c->scheduler), maxProc->context); //switches from the scheduler context to the process's 
      //maxProc->priority *= 0.95; 
...
void procTable(void){
  ...
  //int i = 0;
  cprintf("----------------------------------------------------\n");
  cprintf("\t Name \t State \t\t PID \t Priority\n");
  cprintf("----------------------------------------------------\n");
  for(p = ptable.proc; p < &ptable.proc[NPROC] /*&& i < 10*/; p++){

    if(p->state == UNUSED)
      continue;
      //cprintf("\t UNUSED ");
    if(p->state == EMBRYO)
      cprintf("\t %s \t EMBRYO \t %d \t %d\n", p->name, p->pid, p->priority);
    else if(p->state == SLEEPING)
      cprintf("\t %s \t SLEEPING \t %d \t %d\n", p->name, p->pid, p->priority);
    else if(p->state == RUNNABLE)
      cprintf("\t %s \t RUNNABLE \t %d \t %d\n", p->name, p->pid, p->priority);
    else if(p->state == RUNNING)
      cprintf("\t %s \t RUNNING \t %d \t %d\n", p->name, p->pid, p->priority);
    else if(p->state == ZOMBIE)
      cprintf("\t %s \t ZOMBIE \t %d \t %d\n", p->name, p->pid, p->priority);
...
void printStat(int pid)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  {
    ...
    if (flag == 0)
      cprintf("This process id is not found.\n");
}

int setPriority(int id ,int pr)
{
    struct proc *p;
    int oldPriority = -1;
...
long long sumT(int start){
    long long sum = 0;

    for(int i = start + 1; i <= 100000000; i++){
        sum += i;
    }    
    return sum;
}
